...
class Vegano extends CondicionPreexistente {
	...
	override validarPreferencias(Usuario usuario) {
		if ( usuario.preferencias.exists[t | t == "Pollo"]
			&& usuario.preferencias.exists[t | t == "Carne"]
			&& usuario.preferencias.exists[t | t == "Chivito"]
			&& usuario.preferencias.exists[t | t == "Chori"]) {
			throw new BusinessException("No puede ingresar Pollo, Carne, Chivito ni Chori
